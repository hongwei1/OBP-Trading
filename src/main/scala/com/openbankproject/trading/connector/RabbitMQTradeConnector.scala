...
/**
 * Minimal RabbitMQ-based TradeConnector placeholder.
 */
class RabbitMQTradeConnector[F[_]: Async](host: String, port: Int, username: String, password: String, exchange: String, state: Ref[F, Map[TradeId, Trade]]) extends TradeConnector[F] {
  override def getConnectorInfo: ConnectorInfo = ConnectorInfo(
    name = "RabbitMQ Trade Connector",
    version = "0.1",
...
object RabbitMQTradeConnector {
  def inMemory[F[_]: Async](host: String, port: Int, username: String, password: String, exchange: String): F[RabbitMQTradeConnector[F]] =
    Ref.of[F, Map[TradeId, Trade]](Map.empty).map(ref => new RabbitMQTradeConnector[F](host, port, username, password, exchange, ref))
}
