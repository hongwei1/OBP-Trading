...
/**
 * Minimal RabbitMQ-based OfferConnector placeholder.
 */
class RabbitMQOfferConnector[F[_]: Async](host: String, port: Int, username: String, password: String, exchange: String, state: Ref[F, Map[OfferId, Offer]]) extends OfferConnector[F] {
  override def getConnectorInfo: ConnectorInfo = ConnectorInfo(
    name = "RabbitMQ Offer Connector",
    version = "0.1",
    description = s"RabbitMQ exchange=$exchange (in-memory storage)",
    ...
    configuration = Map("host" -> host, "port" -> port.toString, "exchange" -> exchange)
  )
...
object RabbitMQOfferConnector {
  def inMemory[F[_]: Async](host: String, port: Int, username: String, password: String, exchange: String): F[RabbitMQOfferConnector[F]] =
    Ref.of[F, Map[OfferId, Offer]](Map.empty).map(ref => new RabbitMQOfferConnector[F](host, port, username, password, exchange, ref))
}
